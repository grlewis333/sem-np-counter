...
#         plt.axis('off')
    ...
    # Use auto-calibration if metadata not present
        ...
        return np.nan,im_raw, np.nan,np.nan,np.nan
...
def plot_shapes(shapes,im,ax,
    ...
    for i,s in enumerate(shapes):
        ...
        ax.plot(s[:,0][:][:,0],s[:,0][:][:,1],'o-',color=cols[i%len(cols)],alpha=.6)
        ax.plot([x1,x2],[y1,y2],'o-',color=cols[i%len(cols)],alpha=.6,label=None)
...
#     ax1.imshow(raw_im,cmap='Greys_r')
    plot_shapes(cnts,raw_im,ax)
    cols = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'tab:brown', 'tab:pink', 'tab:gray', 'tab:olive', 'tab:cyan']
    for i in range(len(cnts)):
        circle1 = plt.Circle(centres[i], radii[i],ec=cols[i%len(cols)],fc=(0,0,0,0),lw=2,ls=':')
        ax.add_patch(circle1)
        ax.text(centres[i][0]-radii[i],centres[i][1],str(i),weight='bold',color=cols[i%len(cols)], bbox=dict(facecolor='w',alpha=.6))
...
def full_count_process(im,raw_im,px,w,h,ax, return_vals = True,
